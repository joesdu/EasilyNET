...
public static class QrCode
{
    ...
    {
        var bytes = QrCoder(text, null, keepWhiteBorderPixelVal, width, format, background, foreground);
        var type = format switch
        {
            SKEncodedImageFormat.Bmp  => "bmp",
            SKEncodedImageFormat.Gif  => "gif",
            SKEncodedImageFormat.Ico  => "x-icon",
    ...
    public static byte[] QrCoder(string text, byte[]? logoImage = null, int keepWhiteBorderPixelVal = -1, int width = 320, SKEncodedImageFormat format = SKEncodedImageFormat.Png, string background = "FFF", string foreground = "000")
    {
        ...
        sKBitmap.Dispose();
        using var data = sKImage.Encode(format, 75);
        return data.ToArray();
    }
    ...
    public static string QrDecoder(byte[] data) => QrDecoder(new MemoryStream(data));
    ...
    public static string QrDecoder(string base64)
    {
        ...
        return QrDecoder(Convert.FromBase64String(data));
    }
}