...
internal sealed class SimNetwork(int seed)
{
    ...
    public bool CanDeliver(string from, string to) => !_isolated.Contains(from) && !_isolated.Contains(to);

    public bool ShouldDrop() => DropRate > 0 && _random.NextDouble() < DropRate;

    public long NextDelayTicks() => _random.Next(MinDelayTicks, MaxDelayTicks + 1);

    public bool ShouldReorder() => _random.NextDouble() < 0.15;

    public SimEnvelope Wrap(string from, string to, RaftMessage message, long dueTick) => new(from, to, message, dueTick);
}
